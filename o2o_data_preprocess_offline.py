...
def process_discount_data(df):
    ...
    df['discount_ratio'] = df['Discount_rate'].astype(str).apply(convert_rate)
    df['discount_man'] = df['Discount_rate'].astype(str).apply(get_discount_man)
    df['discount_jian'] = df['Discount_rate'].astype(str).apply(get_discount_jian)
    df['discount_type'] = df['Discount_rate'].astype(str).apply(get_discount_type)
...
def get_user_feature(df):
    ...
    # u_min_distance
    utmp = df[(df['Date'] != -1) & (df['Date_received'] != -1)][['User_id', 'Distance']].copy()
...
def get_merchant_feature(df):
    ...
    m2 = df[df['Date'] != -1][['Merchant_id']].copy()
    m2['m_sale_count'] = 1
    ...
    # m_min_distance
    mtmp = df[(df['Date'] != -1) & (df['Date_received'] != -1)][['Merchant_id', 'Distance']].copy()
...
def get_user_merchant_feature(df):
    ...
    um2 = df[df['Date'] != -1][['User_id', 'Merchant_id']].copy()
    um2['um_buy_count'] = 1
...
def feature_combine_process(feature):
    """
...
if __name__ == '__main__':
    ...
    # discount process
    dfoff = process_discount_data(dfoff)
    ...
    dfoff['date_buy_weekday'] = dfoff['Date'].astype(str).apply(getWeekday)
    dfoff['date_buy_weekday_type'] = dfoff['date_buy_weekday'].apply(lambda x: 1 if x in [6, 7] else 0)  # weekday_type :  周六和周日为1，其他为0
